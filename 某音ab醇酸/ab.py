...
def generate_fingerprint_50(ua_hash, timestamp_ms, aid=6383, page_id=6241):
    fp = [0] * 50
...
def generate_a_bogus(ua, url_params, timestamp_ms=None, ms_token="", aid=6383, page_id=6241):
    ...
    fingerprint_50 = generate_fingerprint_50(ua_hash, timestamp_ms, aid, page_id)
...
def generate_a_bogus_fixed(ua, url_params, timestamp_ms, ms_token=""):
    full_params = url_params + "&msToken=" + ms_token if ms_token else url_params
    url_hash_input = full_params + "dhzx"
    hash1 = sm3_hash_bytes(url_hash_input.encode('utf-8'))
    url_hash = sm3_hash_bytes(hash1)
    ...
    convergence_value = accum
    fingerprint_50 = generate_fingerprint_50(ua_hash, timestamp_ms)
    middle_95 = fingerprint_50 + env_array + ts_array + [convergence_value]

    confusion_randoms = [
        0.5025330655691664, 0.24455694220368707, 0.839034957127988,
        0.07334339022957903, 0.8379466243003488, 0.7481930848031395,
        0.6540951585697395, 0.3987498584844912, 0.63046391699755,
        0.49433810278645407, 0.20946082883542805, 0.15330664142071015,
        0.723844026705413, 0.7602929683042521, 0.7667593599163106,
        0.8087209530404189, 0.8334824601719397, 0.10898092830626971,
        0.893101417613782, 0.9932406096814327, 0.4229780650833517,
        0.3051922321189685, 0.06102092462981357, 0.7532730353261845,
        0.5626730628621242, 0.473835231293136, 0.15218559044339164,
        0.6897737749672014, 0.7838312864934753, 0.729070371764179,
        0.09622458135272971
    ]
    ...
    i = 0
    while i + 2 < len(middle_95):
        rand_val = confusion_randoms[rand_idx % len(confusion_randoms)]
        r = int(rand_val * 1000) & 255
    ...
    prefix_rand = 0.6133344895118178
    prefix_val = int(prefix_rand * 40)
...
if __name__ == "__main__":
    ...
    print("=== 测试1: 使用固定随机数验证 ===")
    result1 = generate_a_bogus_fixed(test_ua, base_url_params, test_timestamp, expected_ms_token)
    print(f"a_bogus: {result1}")

    expected = 'Oy4RgwS7OZ5jKdMbuKTB7vPU4/DArsSy6aTQSiaTyBR0P1UcBbPCBacbaowuBsNHIbphielHDDG/YVxbuGXwZK9pzmpDuMwyytdCV0sLgqHhTTi2IHSTSjGxLvsxUSGYTKQliaiX6t0FILQ3gpKZAFO9SKFPQubMQZN6drUlyx2-g4JY9d5cCPhk'
    print(f"匹配期望: {result1 == expected}")

    print("\n=== 测试2: 使用真实随机数生成 ===")
    result2 = generate_a_bogus(test_ua, base_url_params, ms_token=expected_ms_token)
    print(f"a_bogus: {result2}")
