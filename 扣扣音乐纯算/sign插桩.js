...
function vmLog(...args) {
    ...
    const rawText = args.map(safeStringify).join(' ');
    // 美化字符串：用 pretty 处理
    const prettyText = args.map(pretty).join(' ');

    vmLogs.push({idx, raw: rawText, pretty: prettyText});
}
...
function funcStr(arg) {
    ...
    try {
        ...
        if (bodyMatch) {
            body = bodyMatch[1].replace(/\s+/g, ' ').trim().slice(0, 50);
...
function funclog(method, thisObj, ...args) {
    ...
    const parts = [`[fog_${cid}]`, '方法名:', method.name || 'anonymous', '调用对象:', thisObj];
    if (args.length === 0) {
        parts.push('参数无');
    } else {
        args.forEach((arg, i) => {
            let argStr = arg === undefined ? '(undefined)' :
                arg === null ? '(null)' :
                    arg === '' ? '(空字符串)' :
                        arg;
            parts.push(`参数${i + 1}:`, argStr);
        });
    }
    vmLog(...parts);
}
...
//===========函数调用结果=============
/**
 * 记录函数调用结果（自动关联最近的 funclog 调用 ID）
 * @param {*} result - 函数返回值
 * @returns {*} result - 原样返回，方便链式使用
 */
function funclogresult(result) {
    const cid = callIdCounter;  // funclog 已经递增过，cid 就是当前调用的序号
 ...
 * @returns {*} 运算结果
 */
function twomath(left, op, right, pc) {
    const operators = {
        // 算术运算
        '+': (a, b) => a + b,
        '-': (a, b) => a - b,
                        ...
                        for (var l, f, h = [o, s, t, this, arguments, c, n, 0], d = void 0, p = e, g = []; ;)
                            try {
                                for (; ;)
                                    switch (n[++p]) {
                                        case 0:
                                            vmLog("创建对象————", "构造函数:", h[n[p + 2]], "参数：", h[n[p + 3]])
                                            h[n[++p]] = new h[n[++p]](h[n[++p]]);
                                            vmLog("构造函数结果————", h[n[p - 2]])
                                            break;
                                        case 1:
                                            vmLog("返回————", h[n[p + 1]])
                                            return h[n[++p]];
                                        ...
                                        case 4:
                                            h[n[++p]] += String.fromCharCode(n[++p]),
                                                ...
                                                h[n[++p]] = h[n[++p]][h[n[++p]]];
                                            logpropassign(h[n[p-1]],h[n[p]],h[n[p-2]])
                                            break;
                                        case 5:
                                            twomath(h[n[p + 2]], ">=", n[p + 3])
                                            h[n[++p]] = h[n[++p]] >= n[++p];
                                            break;
                                        case 6:
                                            twomath(h[n[p + 2]], ">>", n[p + 3])
                                        ...
                                        case 13:
                                            ...
                                            h[n[++p]] = h[n[++p]] | h[n[++p]],
                                                h[n[++p]][h[n[++p]]] = h[n[++p]],
                                                logassign(h[n[p-2]],h[n[p-1]],h[n[p]])
                                                p += h[n[++p]] ? n[++p] : n[(++p,
                                                    ++p)];
                                            break;
                                        case 14:
                                            logassign(h[n[p+1]],null,d)
                                            h[n[++p]] = d;
                                            break;
                                        case 15:
                                            h[n[++p]] = n[++p],
                                                h[n[++p]] = h[n[++p]][n[++p]],
                                                logpropassign(h[n[p-1]],n[p],h[n[p-2]]),
                                                h[n[++p]] = n[++p];
                                            break;
                                        case 16:
                                            logassign(h[n[p+1]],null,!0)
                                            h[n[++p]] = !0;
                                            break;
                                        case 17:
                                            twomath(h[n[p + 2]], "===", h[n[p + 3]])
                                        ...
                                        case 18:
                                            ...
                                            break;
                                        case 19:
                                            logassign(h[n[p+1]],h[n[p+2]],h[n[p+3]])
                                            h[n[++p]][h[n[++p]]] = h[n[++p]],
                                                h[n[++p]] = "",
                                                h[n[++p]] += String.fromCharCode(n[++p]);
                                            break;
                                        case 20:
                                            logassign(h[n[p+1]],h[n[p+2]],h[n[p+3]])
                                            h[n[++p]][n[++p]] = h[n[++p]],
                                                logassign(h[n[p+1]],n[p+2],h[n[p+3]]),
                                                h[n[++p]][n[++p]] = h[n[++p]],
                                                logassign(h[n[p+1]],n[p+2],h[n[p+3]]),
                                                h[n[++p]][n[++p]] = h[n[++p]];
                                            break;
                                        ...
                                        case 22:
                                            onemath("++",h[n[p+2]])
                                            h[n[++p]] = ++h[n[++p]],
                                                logassign(h[n[p+1]],null,h[n[p+2]]),
                                                h[n[++p]] = h[n[++p]];
                                            break;
                                        case 23:
                                            h[n[++p]] += String.fromCharCode(n[++p]),
                                        ...
                                        case 29:
                                            ...
                                            break;
                                        case 30:
                                            funclog(h[n[p + 2]], d, h[n[p + 3]], h[n[p + 4]])
                                            h[n[++p]] = h[n[++p]].call(d, h[n[++p]], h[n[++p]]);
                                            funclogresult(h[n[p - 3]])
                                            break;
                                        case 31:
                                            logassign(h[n[p+1]],null,n[p+2])
                                            h[n[++p]] = n[++p],
                                                logassign(h[n[p+1]],null,n[p+2]),
                                                h[n[++p]] = n[++p],
                                                logassign(h[n[p+1]],null,n[p+2]),
                                        ...
                                        case 49:
                                            ...
                                            break;
                                        case 50:
                                            funclog(h[n[p + 2]], h[n[p + 3]])
                                            h[n[++p]] = h[n[++p]].call(h[n[++p]]);
                                            funclogresult(h[n[p - 2]])
                                            break;
                                        case 51:
                                            twomath(h[n[p + 2]], "^", h[n[p + 3]])
                                            h[n[++p]] = h[n[++p]] ^ h[n[++p]];
                                        ...
                                        case 66:
                                            ...
                                            break;
                                        case 67:
                                            funclog(h[n[p + 2]], h[n[p + 3]], h[n[p + 4]])
                                        ...
                                        case 71:
                                            ...
                                            break;
                                        case 72:
                                            funclog(h[n[p + 2]], h[n[p + 3]], h[n[p + 4]], h[n[p + 5]])
                                            h[n[++p]] = h[n[++p]].call(h[n[++p]], h[n[++p]], h[n[++p]]);
                                            funclogresult(h[n[p - 4]])
                                            break;
                                        case 73:
                                            twomath(h[n[p + 2]], ">>", n[p + 3])
                                            h[n[++p]] = h[n[++p]] >> n[++p];
